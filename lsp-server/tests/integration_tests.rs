...
fn test_full_workflow_rust_to_ts() {
    ...
    assert!(locations.len() >= 2, "Should have at least definition and call");
...
fn test_full_workflow_events() {
    ...
    let all_event_names: Vec<String> = index.map
        .iter()
        .filter_map(|entry| {
...
fn test_multi_language_project() {
    ...
    let locations = index.get_locations(lsp_server::syntax::EntityType::Command, "greet");
    assert!(locations.len() >= 2, "Should find command across multiple languages");
}
