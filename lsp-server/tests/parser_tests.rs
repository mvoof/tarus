...
mod rust_parser_tests {
    ...
    fn test_parse_rust_multi_attr_commands() {
        ...
        assert_eq!(
            definitions.len(),
            3,
            "Expected 3 command definitions, got {:?}",
            definitions.iter().map(|f| &f.key).collect::<Vec<_>>()
        );

        let names: Vec<&str> = definitions.iter().map(|f| f.key.as_str()).collect();
        assert!(
            names.contains(&"single_extra_attr"),
            "Missing single_extra_attr"
        );
        assert!(
            names.contains(&"multiple_extra_attrs"),
            "Missing multiple_extra_attrs"
        );
...
mod typescript_parser_tests {
    ...
    fn test_parse_ts_generic_invoke() {
        ...
        let get_user = invokes
            .iter()
            .find(|f| f.key == "get_user")
            .expect("get_user finding");
        assert_eq!(
            get_user.return_type.as_deref(),
            Some("User"),
            "Expected return_type=User for invoke<User>"
        );

        let save_data = invokes
            .iter()
            .find(|f| f.key == "save_data")
            .expect("save_data finding");
        assert_eq!(
            save_data.return_type.as_deref(),
            Some("void"),
            "Expected return_type=void for invoke<void>"
        );
    }
