...
use crate::syntax::{Behavior, EntityType, RustTypeInfo};
...
pub struct FileIndex {
    ...
    /// Rust type definitions (structs, enums) extracted from this file
    pub rust_types: Vec<(String, RustTypeInfo)>,
}
...
pub struct ProjectIndex {
    ...
    /// Maps `camelCase` method names to `snake_case` command names.
    /// e.g., `getUserProfile` → `get_user_profile`
    /// Auto-populated from Rust command definitions (enables `commands.method()` navigation)
    pub method_map: DashMap<String, String>,
    /// Native Rust type registry: type name → full type info from Rust source
...
impl ProjectIndex {
    ...
    pub fn add_file(&self, file_index: FileIndex) {
        ...
        for finding in file_index.findings {
            ...
            let behavior = finding.behavior;

            // Populate method_map for Command definitions (camelCase → snake_case navigation)
            if entity == EntityType::Command && behavior == Behavior::Definition {
                let camel = crate::syntax::snake_to_camel(&finding.key);
                self.method_map
                    .entry(camel)
                    .or_insert_with(|| finding.key.clone());
            }
        ...
        // Populate native rust_types registry
        let mut type_names_for_file = Vec::new();
        for (type_name, type_info) in file_index.rust_types {
            self.rust_types.insert(type_name.clone(), type_info);
            type_names_for_file.push(type_name);
        }
        if !type_names_for_file.is_empty() {
            self.rust_type_paths
                .insert(path_ref.clone(), type_names_for_file);
        }
    ...
    pub fn remove_file(&self, path: &PathBuf) {
        ...
        // Remove native rust_types defined in this file
        if let Some((_, type_names)) = self.rust_type_paths.remove(path) {
            for type_name in type_names {
                self.rust_types.remove(&type_name);
            }
        }
