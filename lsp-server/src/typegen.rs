...
pub fn generate_invoke_types(project_index: &ProjectIndex) -> String {
    ...
    let (mut commands, sorted_events) = collect_commands_and_events(project_index);
...
fn collect_commands_and_events(project_index: &ProjectIndex) -> (Vec<CommandInfo>, Vec<String>) {
            ...
            EntityType::Command => {
                    ...
                    .find(|l| l.behavior == Behavior::Definition)
                {
                    commands.push(CommandInfo {
                        name: key.name.clone(),
                        parameters: def.parameters.clone().unwrap_or_default(),
...
fn generate_custom_type_interfaces(
    project_index: &ProjectIndex,
    output: &mut String,
    mut discovered_types: HashSet<String>,
...
fn generate_command_arg_interfaces(output: &mut String, commands: &[CommandInfo]) {
